...
llama_token llama_model_dspark_noise_token(const struct llama_model * model) {
    return (llama_token) model->hparams.dspark_noise_token;
}

bool llama_model_dspark_markov_bias(const struct llama_model * model, llama_token prev, float * dst) {
    ...
    // lazy CPU cache of w2 (bf16 -> f32), ~n_vocab*rank*4 bytes; callers are single-threaded
    ...
    // markov embedding of the previous token: one w1 row
    ...
    // dst[v] = dot(w2[v], r), sliced across threads
    const int n_threads = std::min<int>(16, std::max<int>(1, (int) std::thread::hardware_concurrency()/4));
    std::vector<std::thread> workers;
    const int64_t chunk = (n_vocab + n_threads - 1) / n_threads;
    for (int t = 0; t < n_threads; ++t) {
        const int64_t v0 = t*chunk;
        const int64_t v1 = std::min(n_vocab, v0 + chunk);
        if (v0 >= v1) {
            break;
        }
        workers.emplace_back([&, v0, v1]() {
